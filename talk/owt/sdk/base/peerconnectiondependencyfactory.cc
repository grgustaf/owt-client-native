...
void PeerConnectionDependencyFactory::
  ...
  // Check all OWT-specific field trials.
  // Only when encoded input is enabled (that we're not doing encoding
  // by ourselves) will we allow SDK to be in low latency mode.
